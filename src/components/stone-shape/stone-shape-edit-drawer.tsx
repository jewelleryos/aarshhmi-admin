...
import { updateStoneShape } from "@/redux/slices/stoneShapeSlice"
...
interface StoneShapeEditDrawerProps {
  stoneShape: StoneShape | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function StoneShapeEditDrawer({ stoneShape, open, onOpenChange }: StoneShapeEditDrawerProps) {
  ...
  // Populate form when drawer opens with stone shape data
  useEffect(() => {
    if (stoneShape && open) {
      setName(stoneShape.name)
      setSlug(stoneShape.slug)
      setDescription(stoneShape.description || "")
      setImageUrl(stoneShape.image_url || "")
      setImageAltText(stoneShape.image_alt_text || "")
      setErrors({})
    }
  }, [stoneShape, open])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Diamond shape updated successfully")
      onOpenChange(false)
    } catch (err) {
      ...
      >
        ...
        <SheetHeader className="text-left px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Edit Diamond Shape</SheetTitle>
              <p className="text-sm text-muted-foreground">
                Update diamond shape details
              </p>
            </div>
          </div>
        </SheetHeader>
