...
import { updatePearlQuality } from "@/redux/slices/pearlQualitySlice"
...
interface PearlQualityEditDrawerProps {
  pearlQuality: PearlQuality | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function PearlQualityEditDrawer({ pearlQuality, open, onOpenChange }: PearlQualityEditDrawerProps) {
  ...
  // Populate form when drawer opens with pearl quality data
  useEffect(() => {
    if (pearlQuality && open) {
      setName(pearlQuality.name)
      setSlug(pearlQuality.slug)
      setDescription(pearlQuality.description || "")
      setImageUrl(pearlQuality.image_url || "")
      setImageAltText(pearlQuality.image_alt_text || "")
      setErrors({})
    }
  }, [pearlQuality, open])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Pearl quality updated successfully")
      onOpenChange(false)
    } catch (err) {
      ...
      >
        ...
        <SheetHeader className="text-left px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Edit Pearl Quality</SheetTitle>
              <p className="text-sm text-muted-foreground">
                Update pearl quality details
              </p>
            </div>
          </div>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <MediaPickerInput
            ...
            rootPath="/masters/pearl-qualities"
