...
interface PearlQualityTableProps {
  items: PearlQuality[]
  onEdit: (item: PearlQuality) => void
  onDelete: (item: PearlQuality) => void
  canUpdate: boolean
  canDelete: boolean
}
...
export function PearlQualityTable({
  items,
  onEdit,
  onDelete,
  canUpdate,
  canDelete,
}: PearlQualityTableProps) {
  // Memoize columns
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 className="text-lg font-medium text-muted-foreground">
          No pearl qualities found
        </h3>
        <p className="text-sm text-muted-foreground mt-1">
          Create your first pearl quality to get started
        </p>
      </div>
    )
  }
