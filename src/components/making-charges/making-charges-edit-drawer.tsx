...
import { updateMakingCharge } from "@/redux/slices/makingChargeSlice"
...
interface MakingChargesEditDrawerProps {
  makingCharge: MakingCharge | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function MakingChargesEditDrawer({
  makingCharge,
  open,
  onOpenChange,
}: MakingChargesEditDrawerProps) {
  ...
  // Populate form when drawer opens with making charge data
  useEffect(() => {
    if (makingCharge && open) {
      setSelectedMetalType(makingCharge.metal_type_id)
      setFrom(makingCharge.from)
      setTo(makingCharge.to)
      setIsFixedPricing(makingCharge.is_fixed_pricing)
      // Amount stored as-is, no conversion needed
      setAmount(makingCharge.amount)
      setErrors({})
    }
  }, [makingCharge, open])
      ...
      >
        ...
        <SheetHeader className="text-left px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Edit Making Charge</SheetTitle>
              <p className="text-sm text-muted-foreground">
                Update making charge details
              </p>
            </div>
          </div>
        </SheetHeader>
