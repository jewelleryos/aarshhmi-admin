...
import { GemstonePricingCSVPreviewTable } from "./gemstone-pricing-csv-preview-table"
import { GemstonePricingUploadResults } from "./gemstone-pricing-upload-results"

interface GemstonePricingUploadDialogProps {
  open: boolean
...
export function GemstonePricingUploadDialog({
  open,
  onOpenChange,
  type,
}: GemstonePricingUploadDialogProps) {
  const dispatch = useAppDispatch()
  const { bulk, filters } = useAppSelector((state) => state.gemstonePricing)
  ...
  const handleFileSelect = useCallback(
    async (selectedFile: File) => {
      ...
      // Parse CSV
      try {
        const parsed = await parseGemstonePricingCSV(selectedFile, type)
  ...
  const title =
    type === "create" ? "Bulk Create Gemstone Prices" : "Bulk Update Gemstone Prices"

  const expectedHeaders =
    type === "create" ? GEMSTONE_CREATE_HEADERS : GEMSTONE_UPDATE_HEADERS
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!w-[90vw] !max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        {step === "upload" && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              {type === "create"
                ? "Upload a CSV file to create new gemstone price entries."
                : "Upload a CSV file to update existing gemstone price entries."}
            </p>
        ...
        {step === "preview" && parsedData && (
          <div className="space-y-4">
            ...
            {!parsedData.headerError && parsedData.rows.length > 0 && (
              <>
                <GemstonePricingCSVPreviewTable
        ...
        {step === "results" && bulk.uploadResult && (
          <GemstonePricingUploadResults result={bulk.uploadResult} type={type} />
        )}
