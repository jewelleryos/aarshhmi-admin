...
interface GemstonePricingCSVPreviewTableProps {
  rows: ParsedRow[]
  type: "create" | "update"
  rowsPerPage?: number
}

export function GemstonePricingCSVPreviewTable({
  rows,
  type,
  rowsPerPage = 10,
}: GemstonePricingCSVPreviewTableProps) {
            ...
            {currentRows.map((row) => (
              <Fragment key={row.rowNumber}>
                ...
                >
                  ...
                  <TableCell>{row.data["gemstone_type"] || "-"}</TableCell>
                  <TableCell>{row.data["shape"] || "-"}</TableCell>
                  <TableCell>{row.data["quality"] || "-"}</TableCell>
                ...
                {!row.isValid && row.errors.length > 0 && (
                  <TableRow className="bg-destructive/5 hover:bg-destructive/5">
                    <TableCell colSpan={type === "update" ? 10 : 9} className="py-2">
