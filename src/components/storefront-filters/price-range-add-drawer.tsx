...
interface PriceRangeAddDrawerProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function PriceRangeAddDrawer({
  open,
  onOpenChange,
}: PriceRangeAddDrawerProps) {
  // Form state
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await storefrontFiltersService.createPriceRange({
      ...
      const message = error.response?.data?.message || "Failed to create price range"
      toast.error(message)
    } finally {
      setIsLoading(false)
    }
      ...
      >
        ...
        <SheetHeader className="text-left px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Add Price Range</SheetTitle>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          {/* Rank */}
          <div className="space-y-2">
            <Label htmlFor="add-rank">Display Order</Label>
            <Input
              id="add-rank"
              type="number"
              min={0}
