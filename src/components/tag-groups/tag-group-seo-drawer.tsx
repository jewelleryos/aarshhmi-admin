...
import { updateTagGroupSeo } from "@/redux/slices/tagGroupSlice"
import { toast } from "sonner"
import type { TagGroup } from "@/redux/services/tagGroupService"

interface TagGroupSeoDrawerProps {
  tagGroup: TagGroup | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function TagGroupSeoDrawer({ tagGroup, open, onOpenChange }: TagGroupSeoDrawerProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Tag group SEO updated successfully")
      onOpenChange(false)
    } catch (err) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {tagGroup?.name} - SEO configuration
              </p>
            </div>
          </div>
        </SheetHeader>
