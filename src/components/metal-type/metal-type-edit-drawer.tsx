...
import { updateMetalType } from "@/redux/slices/metalTypeSlice"
...
interface MetalTypeEditDrawerProps {
  metalType: MetalType | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function MetalTypeEditDrawer({ metalType, open, onOpenChange }: MetalTypeEditDrawerProps) {
  ...
  // Populate form when drawer opens with metal type data
  useEffect(() => {
    if (metalType && open) {
      setName(metalType.name)
      setSlug(metalType.slug)
      setDescription(metalType.description || "")
      setImageUrl(metalType.image_url || "")
      setImageAltText(metalType.image_alt_text || "")
      setErrors({})
    }
  }, [metalType, open])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Metal type updated successfully")
      onOpenChange(false)
    } catch (err) {
      ...
      >
        ...
        <SheetHeader className="text-left px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Edit Metal Type</SheetTitle>
              <p className="text-sm text-muted-foreground">
                Update metal type details
              </p>
            </div>
          </div>
        </SheetHeader>
