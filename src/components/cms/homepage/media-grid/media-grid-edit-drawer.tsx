...
import type { MediaGridItem } from '@/redux/services/cmsService'

interface MediaGridEditDrawerProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  item: MediaGridItem | null
  onSave: (item: MediaGridItem) => Promise<void>
}

export function MediaGridEditDrawer({
  open,
  onOpenChange,
  item,
  onSave,
}: MediaGridEditDrawerProps) {
  // Form state
  ...
  const [errors, setErrors] = useState<{ media_url?: string; redirect_url?: string }>({})

  // Populate form when item changes
  useEffect(() => {
    if (item) {
      setMediaUrl(item.media_url || '')
      setMediaAltText(item.media_alt_text || '')
  ...
  const handleSubmit = async () => {
    ...
    // Validate
    const newErrors: { media_url?: string; redirect_url?: string } = {}
    if (!mediaUrl) {
      newErrors.media_url = 'Media is required'
    }
    if (!redirectUrl) {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <MediaPickerInput
            ...
            onChange={(path) => {
              setMediaUrl(path || '')
              if (path) setErrors((prev) => ({ ...prev, media_url: undefined }))
            }}
            rootPath="cms/homepage/media-grid"
            required
          ...
          <div className="space-y-2">
            <Label htmlFor="edit_media_alt_text">Media Alt Text</Label>
            <Input
              id="edit_media_alt_text"
