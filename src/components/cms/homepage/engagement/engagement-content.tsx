...
import { cmsService, type EngagementContent } from '@/components/cms/services/cmsService'
...
export function EngagementContentComponent() {
  ...
  const fetchContent = async () => {
    try {
      setIsLoading(true)
      const response = await cmsService.getEngagement()
  ...
  const handleSave = async () => {
    ...
    if (!buttonText) newErrors.button_text = 'Button text is required'
    if (!redirectUrl) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage the engagement section on homepage
          </p>
        </div>
      ...
      <Card>
        <CardContent className="pt-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="redirect_url"
              placeholder="https://example.com/engagement"
              value={redirectUrl}
              onChange={(e) => {
            ...
            {errors.redirect_url ? (
              ...
              <p className="text-xs text-muted-foreground">
                Full URL with https (e.g., https://example.com/engagement)
              </p>
            )}
          </div>
        </CardContent>
