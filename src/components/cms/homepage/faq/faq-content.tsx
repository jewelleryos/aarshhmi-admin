...
import { cmsService, type FAQItem, type FAQContent } from '@/components/cms/services/cmsService'
...
export function FAQContentComponent() {
  ...
  const handleAddItem = async (item: Omit<FAQItem, 'id'>) => {
    try {
      const newItem: FAQItem = { ...item, id: `faq_${Date.now()}` }
  ...
  const handleToggleStatus = async (itemId: string) => {
    try {
      const updatedItems = items.map((i) => (i.id === itemId ? { ...i, status: !i.status } : i))
      const response = await cmsService.updateFAQ({ items: updatedItems })
      toast.success(response.message)
      setItems(updatedItems)
    } catch (err: unknown) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage homepage frequently asked questions</p>
        </div>
        ...
        <CardContent className="pt-6">
          {sortedItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Click &quot;Add FAQ&quot; to create your first question</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center">
                    <AccordionTrigger className="flex-1 text-left hover:no-underline">
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-muted-foreground font-normal">#{item.rank}</span>
      ...
      <FAQAddDrawer open={isAddDrawerOpen} onOpenChange={setIsAddDrawerOpen} onSave={handleAddItem} />
      <FAQEditDrawer open={isEditDrawerOpen} onOpenChange={setIsEditDrawerOpen} item={selectedItem} onSave={handleEditItem} />
    </div>
  )
}
