...
import { cmsService, type FAQsSectionItem, type FAQsSectionContent } from '@/components/cms/services/cmsService'
...
export function FAQsContentComponent() {
  ...
  const handleAddItem = async (item: Omit<FAQsSectionItem, 'id'>) => {
    try {
      const newItem: FAQsSectionItem = { ...item, id: `faq_${Date.now()}` }
  ...
  return (
    <div className="space-y-6">
      ...
      <FAQsAddDrawer open={isAddDrawerOpen} onOpenChange={setIsAddDrawerOpen} onSave={handleAddItem} />
      <FAQsEditDrawer open={isEditDrawerOpen} onOpenChange={setIsEditDrawerOpen} item={selectedItem} onSave={handleEditItem} />
    </div>
  )
}
