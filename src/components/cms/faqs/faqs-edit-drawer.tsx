...
import type { FAQsSectionItem } from '@/components/cms/services/cmsService'

interface FAQsEditDrawerProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  item: FAQsSectionItem | null
  onSave: (item: FAQsSectionItem) => Promise<void>
}

export function FAQsEditDrawer({ open, onOpenChange, item, onSave }: FAQsEditDrawerProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent className="sm:max-w-xl flex flex-col p-0" onOpenAutoFocus={(e) => e.preventDefault()}>
        <SheetHeader className="text-left px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Update the FAQ item</p>
            </div>
          </div>
        </SheetHeader>
