...
import { type LargeSizeBannersItem, type CategoryWithChildrenForSelect } from '../../services/cmsService'

interface LargeSizeBannersTableProps {
  items: LargeSizeBannersItem[]
  categories: CategoryWithChildrenForSelect[]
  onEdit: (item: LargeSizeBannersItem) => void
...
function createColumns(
  ...
  onDelete: (itemId: string) => void,
  onToggleStatus: (itemId: string) => void
): ColumnDef<LargeSizeBannersItem>[] {
  return [
    {
      accessorKey: 'rank',
      header: ({ column }) => (
...
export function LargeSizeBannersTable({
  items,
  categories,
  onEdit,
  onDelete,
  onToggleStatus,
}: LargeSizeBannersTableProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Click &quot;Add Banner&quot; to create your first large size banner
        </p>
      </div>
    )
  }
