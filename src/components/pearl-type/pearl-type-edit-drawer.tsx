...
import { updatePearlType } from "@/redux/slices/pearlTypeSlice"
...
interface PearlTypeEditDrawerProps {
  pearlType: PearlType | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function PearlTypeEditDrawer({ pearlType, open, onOpenChange }: PearlTypeEditDrawerProps) {
  ...
  // Populate form when drawer opens with pearl type data
  useEffect(() => {
    if (pearlType && open) {
      setName(pearlType.name)
      setSlug(pearlType.slug)
      setDescription(pearlType.description || "")
      setImageUrl(pearlType.image_url || "")
      setImageAltText(pearlType.image_alt_text || "")
      setErrors({})
    }
  }, [pearlType, open])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Pearl type updated successfully")
      onOpenChange(false)
    } catch (err) {
      ...
      >
        ...
        <SheetHeader className="text-left px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Edit Pearl Type</SheetTitle>
              <p className="text-sm text-muted-foreground">
                Update pearl type details
              </p>
            </div>
          </div>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="edit-name"
              placeholder="Enter pearl type name"
              value={name}
