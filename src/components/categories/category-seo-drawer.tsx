...
import { updateCategorySeo } from "@/redux/slices/categorySlice"
...
export function CategorySeoDrawer({
  category,
  open,
  onOpenChange,
}: CategorySeoDrawerProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Category SEO updated successfully")
      onOpenChange(false)
    } catch (err) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {category?.name} - SEO configuration
              </p>
            </div>
          </div>
        </SheetHeader>
