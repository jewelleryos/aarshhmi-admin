...
import { updateDiamondPrice } from "@/redux/slices/diamondPricingSlice"
import { DiamondPrice, DropdownItem } from "@/redux/services/diamondPricingService"
import { toSmallestUnit, fromSmallestUnit } from "@/utils/currency"
import { toast } from "sonner"

interface DiamondPricingEditDrawerProps {
...
export function DiamondPricingEditDrawer({
  diamondPrice,
  open,
  onOpenChange,
  shapes,
  qualities,
}: DiamondPricingEditDrawerProps) {
  ...
  // Populate form when diamondPrice changes
  useEffect(() => {
    if (diamondPrice && open) {
      setShapeId(diamondPrice.stone_shape_id)
      setQualityId(diamondPrice.stone_quality_id)
      setCtFrom(diamondPrice.ct_from.toString())
      setCtTo(diamondPrice.ct_to.toString())
      setPrice(fromSmallestUnit(diamondPrice.price).toString())
      setErrors({})
    }
  }, [diamondPrice, open])
      ...
      >
        ...
        <SheetHeader className="text-left px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Edit Diamond Price</SheetTitle>
              <p className="text-sm text-muted-foreground">
                Update pricing entry for lab-grown diamonds
              </p>
            </div>
          </div>
        </SheetHeader>
