...
import { updateGemstoneType } from "@/redux/slices/gemstoneTypeSlice"
...
interface GemstoneTypeEditDrawerProps {
  gemstoneType: GemstoneType | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function GemstoneTypeEditDrawer({ gemstoneType, open, onOpenChange }: GemstoneTypeEditDrawerProps) {
  ...
  // Populate form when drawer opens with gemstone type data
  useEffect(() => {
    if (gemstoneType && open) {
      setName(gemstoneType.name)
      setSlug(gemstoneType.slug)
      setDescription(gemstoneType.description || "")
      setImageUrl(gemstoneType.image_url || "")
      setImageAltText(gemstoneType.image_alt_text || "")
      setErrors({})
    }
  }, [gemstoneType, open])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Gemstone type updated successfully")
      onOpenChange(false)
    } catch (err) {
      ...
      >
        ...
        <SheetHeader className="text-left px-6 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle>Edit Gemstone Type</SheetTitle>
              <p className="text-sm text-muted-foreground">
                Update gemstone type details
              </p>
            </div>
          </div>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <MediaPickerInput
            ...
            rootPath="/masters/gemstone-type"
